...
#         Shape after first conv: torch.Size([20, 10, 26, 26])
#         Shape after first pool: torch.Size([20, 10, 13, 13])
#         Shape after secnd pool: torch.Size([20, 10, 12, 12])
#         Shape after flattening: torch.Size([10, 2880])
#         size mismatch, m1: [10 x 2880], m2: [10 x 10]
        ...
        # 10 input image channel 10 output channels/feature maps
    ...
    def forward(self, x):
        ...
        p = self.pool1(x)
        # two activate conv layers with pooling from first
        x1 = F.relu(self.conv2(p))
        # flatten for linear layer
        x1 = x1.view(x1.size(0), -1)
        # dropout with 0.3 prob
        x1 = self.drop(x1)
        x2 = F.relu(self.linear(x1))
        # final output
#         pred = nn.functional.softmax(x2)
        return x2
